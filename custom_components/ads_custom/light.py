...
async def async_setup_entry(
    ...
    for subentry_id, subentry, _, _, entity_data in iter_subentry_entities(entry):
        if subentry.subentry_type != SUBENTRY_TYPE_ENTITY:
            continue
        if entity_data.get("entity_type") != "light":
            continue

        name = entity_data.get(CONF_NAME, DEFAULT_NAME)
        ads_var = entity_data.get(CONF_ADS_VAR)
        ads_var_brightness = entity_data.get(CONF_ADS_VAR_BRIGHTNESS)
        brightness_scale = entity_data.get(CONF_ADS_BRIGHTNESS_SCALE, DEFAULT_BRIGHTNESS_SCALE)
        brightness_type = entity_data.get(CONF_ADS_VAR_BRIGHTNESS_TYPE, DEFAULT_BRIGHTNESS_TYPE)
